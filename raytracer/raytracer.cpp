...
struct Sphere {
    ...
    // constructeur
    Sphere(double r, Vector p, Vector e, Vector c, SurfaceType m) : radius(r), position(p), emission(e), color(c),
                                                                    material(m) {}
...
Vector compute_radiance(const Ray &ray, int depth) {
    ...
    if (obj.material == SurfaceType::diffuse) {
        ...
        radiance = obj.emission + f.multiply(depth > 2 ? (random01(rng) < p ?
                                                          compute_radiance(reflection_ray, depth) * rp :
...
void save_image_file(int width, int height, int ray_per_pixel, const Vector *pixel) {
    ...
    std::stringstream ss;
    ss << "../bin/data/" << "image" << width << "x" << height << ".ppm";
...
void init() {
    ...
    scene.insert(scene.begin(), {
            ...
            Sphere(wall_radius, Vector(box_center_x, anchor, box_size_z), Vector(), Vector(0.75, 0.75, 0.75),
                   SurfaceType::diffuse),    // plancher
            Sphere(wall_radius, Vector(box_center_x, -anchor + box_size_y, box_size_z), Vector(),
                   Vector(0.75, 0.75, 0.75), SurfaceType::diffuse),    // plafond
            Sphere(wall_radius, Vector(anchor + 1, box_center_y, box_size_z), Vector(), Vector(0.75, 0.25, 0.25),
                   SurfaceType::diffuse),    // mur gauche
            Sphere(wall_radius, Vector(box_center_x, box_center_y, anchor), Vector(), Vector(0.25, 0.25, 0.25),
                   SurfaceType::specular),    // mur arrière
            Sphere(wall_radius, Vector(-anchor + 99, box_center_y, box_size_z), Vector(), Vector(0.25, 0.25, 0.75),
                   SurfaceType::diffuse),    // mur droit
            Sphere(wall_radius, Vector(box_center_x, box_center_y, -anchor + 170), Vector(), Vector(0.0, 0.0, 0.0),
                   SurfaceType::diffuse),    // mur avant

            // ensemble des sphères situées à l'intérieur de la boîte de Cornell
            //Sphere(22.5,        Vector( 30,            30,                40),           Vector(), Vector(1.0, 1.0, 1.0),    SurfaceType::diffuse),   // sphère mirroir
            Sphere(17.5, Vector(75, 40, 75), Vector(), Vector(1.0, 1.0, 1.0), SurfaceType::diffuse), // sphère de verre

            Sphere(600, Vector(box_center_x, 600.0 + box_size_z - 0.27, box_size_z), Vector(15, 15, 15),
                   Vector(0.0, 0.0, 0.0), SurfaceType::diffuse) // sphère lumineuse
    });
