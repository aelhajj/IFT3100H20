...
enum class CurveType {
    bezier_quadratic, bezier_cubic, bezier_surface
};
    ...
    for (int i = 0; i < ctrl_pointx.size(); i++) {
        ...
        for (int j = 0; j < ctrl_pointx[i].size(); j++) {
            x += (factorial(n) / (factorial(i) * factorial(n - i))) *
                 (pow(u, i) * pow(1 - u, n - i) * (factorial(m) / (factorial(j) * factorial(m - j)))) *
                 (pow(v, j) * pow(1 - v, m - j)) * (ctrl_pointx[i][j] + animation_float);
            y += (factorial(n) / (factorial(i) * factorial(n - i))) *
                 (pow(u, i) * pow(1 - u, n - i) * (factorial(m) / (factorial(j) * factorial(m - j)))) *
                 (pow(v, j) * pow(1 - v, m - j)) * (ctrl_pointy[i][j] + animation_float);
            z += (factorial(n) / (factorial(i) * factorial(n - i))) *
                 (pow(u, i) * pow(1 - u, n - i) * (factorial(m) / (factorial(j) * factorial(m - j)))) *
                 (pow(v, j) * pow(1 - v, m - j)) * (ctrl_pointz[i][j] + animation_float);
        }
    }
}
